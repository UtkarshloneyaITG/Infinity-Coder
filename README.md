...
A self-contained AI coding agent for Visual Studio Code. It reads, writes, searches
and runs code in your project — and it does it with **your** API key, against **any**
...
3. Reload VS Code. The 🏹 **Infinity Coder** icon appears in the activity bar.
...
npm run compile
```
Open the folder in VS Code and press **`F5`**. A second VS Code window opens with the
extension loaded.
...
**Your keys are stored in your operating system's keychain** via VS Code's
`SecretStorage`. They never touch `settings.json`, never appear in the UI beyond the
last 4 characters, and are deleted from the keychain when you remove them.
...
- **Skills** — reusable `SKILL.md` instruction files (see below).
...
- **Live activity strip** — what the agent is doing right now, elapsed time, and a Stop button.
